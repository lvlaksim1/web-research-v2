...
internal object WebResearchScripts {
    fun instrumentation(): String = """
          (function(){
            ...
            const elementPath=e=>{try{if(!e||e.nodeType!==1)return'';if(e.id)return'#'+CSS.escape(e.id);const parts=[];let n=e;while(n&&n.nodeType===1&&parts.length<6){let p=(n.tagName||'').toLowerCase();const parent=n.parentElement;if(parent){const same=Array.from(parent.children).filter(x=>x.tagName===n.tagName);if(same.length>1)p+=':nth-of-type('+(same.indexOf(n)+1)+')'}parts.unshift(p);n=parent}return parts.join(' > ')}catch(e){return''}};
            const nodeSummary=n=>{try{return{nodeType:n.nodeType,name:n.nodeName||'',text:(n.textContent||'').trim().slice(0,500),html:n.outerHTML?String(n.outerHTML).slice(0,2000):'',selector:n.nodeType===1?elementPath(n):'',parent:n.parentElement?elementPath(n.parentElement):''}}catch(e){return{name:n?.nodeName||''}}};
            let mutationTimer=0,mutationQueue=[],mutationDropped=0;
            const flushMutations=()=>{mutationTimer=0;if(!mutationQueue.length&&!mutationDropped)return;send({source:'dom-mutation',time:Date.now(),page:location.href,mutations:mutationQueue,dropped:mutationDropped});mutationQueue=[];mutationDropped=0};
            new MutationObserver(ms=>{for(const m of ms){let item={type:m.type,target:m.target?.nodeType===1?elementPath(m.target):'',parent:m.target?.parentElement?elementPath(m.target.parentElement):''};if(m.type==='attributes'){item.attribute=m.attributeName||'';item.oldValue=m.oldValue;try{item.newValue=m.target?.getAttribute?.(m.attributeName)||null}catch(e){item.newValue=null}}else if(m.type==='characterData'){item.oldValue=m.oldValue;item.newValue=String(m.target?.data||'').slice(0,2000)}else{item.added=Array.from(m.addedNodes||[]).slice(0,20).map(nodeSummary);item.removed=Array.from(m.removedNodes||[]).slice(0,20).map(nodeSummary);for(const n of Array.from(m.addedNodes||[])){if(!n||n.nodeType!==1)continue;if(String(n.tagName||'').toLowerCase()==='script')archiveScript(n,location.href+'#inline-dynamic-'+(++dynamicInline));try{if(n.querySelectorAll)n.querySelectorAll('script').forEach(s=>archiveScript(s,location.href+'#inline-dynamic-'+(++dynamicInline)))}catch(e){}}}if(mutationQueue.length<120)mutationQueue.push(item);else mutationDropped++}if(!mutationTimer)mutationTimer=setTimeout(flushMutations,500)}).observe(document.documentElement,{subtree:true,childList:true,attributes:true,attributeOldValue:true,characterData:true,characterDataOldValue:true});
            addEventListener('error',e=>{if(e.target&&e.target!==window){send({source:'resource-error',time:Date.now(),url:e.target.src||e.target.href||'',tag:(e.target.tagName||'').toLowerCase(),page:location.href});return}send({source:'js-error',time:Date.now(),message:e.message,url:e.filename||location.href,line:e.lineno||0,column:e.colno||0,stack:e.error?.stack||''})},true);
            addEventListener('unhandledrejection',e=>send({source:'promise-rejection',time:Date.now(),message:String(e.reason),stack:e.reason?.stack||''}));
            addEventListener('securitypolicyviolation',e=>send({source:'csp-violation',time:Date.now(),blockedURI:e.blockedURI||'',violatedDirective:e.violatedDirective||'',effectiveDirective:e.effectiveDirective||'',sourceFile:e.sourceFile||'',line:e.lineNumber||0,column:e.columnNumber||0,statusCode:e.statusCode||0}));
            try{if(window.PerformanceObserver){new PerformanceObserver(list=>{for(const r of list.getEntries()){send({source:'resource-timing',time:Date.now(),method:'GET',url:r.name||'',initiatorType:r.initiatorType||'',duration:r.duration||0,timing:{startTime:r.startTime||0,redirectStart:r.redirectStart||0,redirectEnd:r.redirectEnd||0,fetchStart:r.fetchStart||0,domainLookupStart:r.domainLookupStart||0,domainLookupEnd:r.domainLookupEnd||0,connectStart:r.connectStart||0,secureConnectionStart:r.secureConnectionStart||0,connectEnd:r.connectEnd||0,requestStart:r.requestStart||0,responseStart:r.responseStart||0,responseEnd:r.responseEnd||0,nextHopProtocol:r.nextHopProtocol||''},transferSize:r.transferSize||0,encodedBodySize:r.encodedBodySize||0,decodedBodySize:r.decodedBodySize||0})}}).observe({type:'resource',buffered:true})}}catch(e){}
            try{const n=performance.getEntriesByType('navigation')?.[0];if(n)send({source:'navigation-timing',time:Date.now(),method:'GET',url:location.href,duration:n.duration||0,timing:{redirectStart:n.redirectStart||0,redirectEnd:n.redirectEnd||0,domainLookupStart:n.domainLookupStart||0,domainLookupEnd:n.domainLookupEnd||0,connectStart:n.connectStart||0,secureConnectionStart:n.secureConnectionStart||0,connectEnd:n.connectEnd||0,requestStart:n.requestStart||0,responseStart:n.responseStart||0,responseEnd:n.responseEnd||0,domInteractive:n.domInteractive||0,domContentLoadedEventEnd:n.domContentLoadedEventEnd||0,loadEventEnd:n.loadEventEnd||0,nextHopProtocol:n.nextHopProtocol||''}})}catch(e){}
            send({source:'hook',time:Date.now(),url:location.href,status:0,instrumentationPhase:'runtime'});
          })();
        """.trimIndent()
